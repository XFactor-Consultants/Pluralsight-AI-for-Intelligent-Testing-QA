...
def make_dataset(n=1000):
    ...
    p = df["coverage_overlap"] * 0.45 + (df["churn"] / 50) * 0.35 + df["historical_fail_rate"] * 0.20
...
if __name__ == "__main__":
    ...
    suite = make_dataset(200)
    suite["failure_prob"] = model.predict_proba(suite[FEATURES])[:, 1]
    suite["actually_fails"] = (np.random.default_rng(99).random(200) < 0.25).astype(int)
    ...
    for label, subset, color in [("ML Ranked", ranked, "#2980b9"), ("Random", random, "#e74c3c")]:
        ...
        plt.plot(np.arange(1, 201) / 200 * 100, cum * 100, label=label, color=color)

    plt.xlabel("% Suite Executed"); plt.ylabel("% Faults Detected")
    plt.title("Fault Detection Curve"); plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout(); plt.savefig("apfd_curve.png", dpi=150)
